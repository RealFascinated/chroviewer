...
import { renderBeatLeaderReplayPreview } from '../../server/preview-image';

const errorHeaders = { 'cache-control': 'no-store', 'content-type': 'text/plain; charset=utf-8' };

export const Route = createFileRoute('/api/preview/replay-bl')({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const result = await renderBeatLeaderReplayPreview(scoreId, url.origin);
        if (result.isErr()) {
